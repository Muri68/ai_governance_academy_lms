...
from apps.courses.models import (
    Course, Lesson, LessonContent, CourseCategory, Enrollment,
    LessonProgress, CourseReview, CourseAnnouncement, FeaturedCourse,
    TopPick, Quiz, QuizQuestion, QuizAnswer, QuizAttempt, QuizAnswerRecord
)
...
def edit_course(request, course_id):
    ...
    for lesson_index, lesson in enumerate(course.lessons.all().order_by('order')):
        lesson_data = {
            'id': lesson.id,
            'title': lesson.title,
            'contents': []
        ...
        for content in lesson.contents.all().order_by('order'):
            ...
            lesson_data['contents'].append(content_item)
        existing_lessons.append(lesson_data)
        
        # Prepare quiz data for this lesson
        ...
        if quiz:
            ...
            existing_quizzes.append(quiz_data)
...
def _admin_save_course(request, is_edit=False, course=None):
    ...
    quizzes_data_str = request.POST.get('quizzes_data', '[]')
    ...
    if is_edit and course:
        ...
        # SAVE quizzes
        _admin_save_quizzes(request, course, quizzes_data_str)
...
def _admin_save_quizzes(request, course, quizzes_data_str):
    """Save quizzes from the custom admin form"""
    try:
        quizzes_data = json.loads(quizzes_data_str)
    ...
    # Track which lessons have quizzes in the submitted data
    submitted_lesson_ids = set()
    
    for quiz_data in quizzes_data:
        lesson_index = quiz_data.get('lesson_index')
        lesson_id = quiz_data.get('lesson_id')
        quiz_title = quiz_data.get('title', '').strip()
        questions_data = quiz_data.get('questions', [])
        passing_score = quiz_data.get('passing_score', 60)
        questions_per_attempt = quiz_data.get('questions_per_attempt')
        randomize_questions = quiz_data.get('randomize_questions', True)
        randomize_answers = quiz_data.get('randomize_answers', True)
        
        # Find the lesson
        lesson = None
        if lesson_id:
            lesson = course.lessons.filter(id=lesson_id).first()
        elif lesson_index is not None and lesson_index < len(lessons):
            lesson = lessons[lesson_index]
        
        if not lesson:
            continue
        
        submitted_lesson_ids.add(lesson.id)
        
        # Skip if no questions AND no existing quiz (don't create empty quiz)
        if not questions_data:
            # Only delete if there was an existing quiz and it's explicitly removed
            existing_quiz = Quiz.objects.filter(lesson=lesson).first()
            if existing_quiz and not quiz_title:
                # Quiz was explicitly removed
        ...
        quiz, created = Quiz.objects.get_or_create(
            lesson=lesson,
            defaults={
                'title': quiz_title or f"Quiz for {lesson.title}",
        ...
        if created:
            quiz.title = quiz_title or f"Quiz for {lesson.title}"
        else:
        ...
        quiz.questions_per_attempt = questions_per_attempt
        quiz.randomize_questions = randomize_questions
        quiz.randomize_answers = randomize_answers
        quiz.save()
        ...
        for q_index, question_data in enumerate(questions_data):
            ...
            # Create answers
            for a_index, answer_data in enumerate(question_data.get('answers', [])):
...
def _admin_update_contents(request, lesson, contents_data, lesson_index):
    ...
    for j, content_data in enumerate(contents_data):
        ...
        if content_id and content_id in existing_content_ids:
            ...
            content.text_content = text_content if content_type in ['text', 'assignment', 'code'] else ''
            content.order = j + 1
            content.save()
        else:
            # CREATE new content
            content = LessonContent.objects.create(
                ...
                text_content=text_content if content_type in ['text', 'assignment', 'code'] else '',
                order=j + 1,
            )
...
# Student-facing views for quiz taking
@login_required
    ...
    for answer_data in answers:
        ...
        if question.question_type == 'short_answer':
            ...
            QuizAnswerRecord.objects.create(
                ...
                is_correct=False,  # Will be reviewed manually
            )
        else:
...
def _get_course(course_id):
    """Helper function to get course or 404"""
...
@login_required
@admin_required
def manage_final_exam(request, course_id):
    """Admin manages final exam for a course"""
    course = _get_course(course_id)
    
    # Get or create final exam
    final_exam, created = FinalExam.objects.get_or_create(
        course=course,
        defaults={
            'title': f"Final Examination - {course.title}",
            'passing_score': 70,
            'time_limit': 30,
            'max_attempts': 3,
        }
    ...
    if request.method == 'POST':
        ...
        if action == 'update_settings':
            ...
            final_exam.title = request.POST.get('title', final_exam.title).strip()
            final_exam.description = request.POST.get('description', '')
            final_exam.passing_score = int(request.POST.get('passing_score', 70))
            final_exam.time_limit = int(request.POST.get('time_limit', 30))
            final_exam.max_attempts = int(request.POST.get('max_attempts', 3))
            final_exam.randomize_questions = request.POST.get('randomize_questions') == 'on'
            final_exam.randomize_answers = request.POST.get('randomize_answers') == 'on'
            
            questions_per_attempt = request.POST.get('questions_per_attempt', '')
            final_exam.questions_per_attempt = int(questions_per_attempt) if questions_per_attempt.strip() else None
            
            final_exam.is_active = request.POST.get('is_active') == 'on'
            final_exam.save()
            
            messages.success(request, 'Final exam settings updated successfully!')
            return redirect('dashboard:manage_final_exam', course_id=course.id)
            ...
            if question_text:
                last_order = final_exam.questions.order_by('-order').first()
                ...
                if question_type in ['multiple_choice', 'true_false']:
                    ...
                    correct_answer_index = request.POST.get('is_correct', '0')
                    
                    for i, answer_text in enumerate(answer_texts):
                        if answer_text.strip():
                            FinalExamAnswer.objects.create(
                                question=question,
            ...
            question = get_object_or_404(FinalExamQuestion, id=question_id, exam=final_exam)
            ...
            # Delete question
            question_id = request.POST.get('question_id')
            FinalExamQuestion.objects.filter(id=question_id, exam=final_exam).delete()
            messages.success(request, 'Question deleted successfully!')
            return redirect('dashboard:manage_final_exam', course_id=course.id)
        
        elif action == 'toggle_active':
            # Toggle exam active status
            final_exam.is_active = not final_exam.is_active
            final_exam.save()
            messages.success(request, f'Final exam {"activated" if final_exam.is_active else "deactivated"} successfully!')
            return redirect('dashboard:manage_final_exam', course_id=course.id)
    ...
    total_attempts = final_exam.attempts.count()
    passed_attempts = final_exam.attempts.filter(passed=True).count()
