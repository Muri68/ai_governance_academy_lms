...
@student_required
def student_certificates(request):
    """Display student's earned certificates - only for courses with passed final exam"""
    from apps.courses.models import Enrollment, FinalExam, FinalExamAttempt
    
    # Get completed enrollments
    completed_enrollments = Enrollment.objects.filter(
        student=request.user,
        status='completed',
        certificate_issued=True  # Only show if certificate was actually issued
    ).select_related('course', 'course__instructor').order_by('-completed_at')
    
    # Filter further to ensure final exam was passed (if course has final exam)
    certificates = []
    
    for enrollment in completed_enrollments:
        # Check if course has final exam
        final_exam = FinalExam.objects.filter(course=enrollment.course, is_active=True).first()
        
        if final_exam:
            # Course has final exam - verify student passed it
            ...
            if passed_exam and enrollment.certificate_issued:
                certificates.append(enrollment)
        else:
            # No final exam - certificate is valid
